...
## GitHub Actions로 자동 IPA 빌드 (추가됨)
1. 이 폴더 전체를 GitHub에 push (main 브랜치)
2. Actions 탭 > Build IPA for SideStore > Run
3. 3~5분 후 Artifacts에서 BookReaderTTS-unsigned.ipa 다운로드
4. SideStore로 설치

SideStore 팁: SideStore 설정에서 WiredGuard VPN 설치 후, 앱에서 Refresh를 켜두면 7일마다 자동 갱신되어 영구 사용 가능.
