...
struct ContentView: View {
    ...
    var body: some View {
        NavigationView {
            VStack(spacing:0) {
                    ...
                    VStack(alignment:.leading, spacing:16) {
                        if !text.isEmpty {
                            ...
                            VStack(spacing:12){
                                ...
                                Text("폴더 아이콘으로\n.txt md docx pdf epub 파일을 여세요").multilineTextAlignment(.center).foregroundColor(.secondary)
                            }.padding(60)
                        }
                    }.padding(.bottom,160)
                }
                ...
                HStack{
                    Text(chapters.isEmpty ? "자동 구분" : "목차 \(chapters.count)개").font(.caption).foregroundColor(.secondary)
                    Spacer()
    ...
    var attributedText: AttributedString {
        ...
        if let r = tts.currentRange {
            let loc = r.location
            let len = r.length
            if loc + len <= attr.characters.count {
                let startIdx = attr.index(attr.startIndex, offsetByCharacters: loc)
                let endIdx = attr.index(startIdx, offsetByCharacters: len)
