...
struct ContentView: View {
        ...
        NavigationView {
            ZStack(alignment:.bottom){
                ScrollView {
                    VStack(alignment:.leading, spacing:20){
                        if !text.isEmpty {
                            highlightedTextView.padding(.horizontal,18).padding(.top,12)
            ...
            .toolbar{
                ToolbarItem(placement:.navigationBarLeading){
                    Button(action:{showPicker=true}){ Image(systemName:"folder.badge.plus").font(.body.bold()) }
                }
    ...
    var emptyView: some View {
        VStack(spacing:16){
            Image(systemName:"books.vertical").font(.system(size:54))
            Text("폴더 아이콘으로\n책 파일을 열어보세요").multilineTextAlignment(.center).foregroundColor(.secondary)
    ...
    var bottomPlayer: some View {
        VStack(spacing:0){
            ...
            HStack(spacing:14){
                Button(action:{ tts.isSpeaking ? tts.pause() : playCurrent() }){
                    Image(systemName: tts.isSpeaking ? "pause.circle.fill" : "play.circle.fill").font(.system(size:42)).foregroundColor(.black)
                }
            ...
            }.padding(14).background(Color(.systemBackground)).shadow(radius:12, y:-4)
        }
    }
